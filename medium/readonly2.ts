...
// type MyReadonlyMy1<T, K extends keyof T> = {
//   [P in keyof T as P extends K ? readonly P : P]: T[P];
// };

// Readonly의 경우 [key] "앞에" 붙여야한다. 즉, 키에 따라 조건문을 비교하며 ReadOnly를 붙일 수 없는 것
// 따라서 분기에 따른 타입을 나누고 합쳐야한다

type MyReadonlyMy2<T, K extends keyof T = keyof T> = {};
// keyof T 라는 기본값을 설정하지 않으면, 항상 두 개의 인자값을 받기를 기대하므로 에러가 발생했다.
